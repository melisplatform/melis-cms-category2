...
class MelisCmsCategoryDocumentController extends AbstractActionController
{
    ...
    {
        $melisKey = $this->params()->fromRoute('melisKey', '');
        $categoryId = $this->getRequest()->getQuery('catId');
        $categoryMediaData = [];
        if (! empty($categoryId)) {
            $categoryService = $this->getServiceLocator()->get('MelisCmsCategory2Service');
            $categoryMediaData = $categoryService->getCategoryMediaById($categoryId)->current();
        }

        $view = new ViewModel();
        $view->melisKey = $melisKey;
        $view->categoryMediaData = $categoryMediaData;
    ...
    public function renderDocumentFileListsAction()
    {
        ...
        $view->setVariable('fileCount', $fileCount);
